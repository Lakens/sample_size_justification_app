...
`r if(params$width_effect_estimate != ""){paste0("an evaluation of effects one has sufficient power to detect based on a sensitivity power analysis as described below:")}`
```{r echo = FALSE, comment = NA}
if(params$sensitivity_power != ""){cat(gsub(pattern = "\n", replacement = "  \n", x = params$sensitivity_power))}
```
