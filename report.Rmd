...
`r if(params$heuristic == "yes"){paste("The sample size was not determine in relation to a specific inferentical goal, but based on heuristics, about which the following information has been provided:")}`

```{r echo = FALSE, comment = NA}
if(params$heuristic == "yes" && params$heuristic_details != ""){cat(gsub(pattern = "\n", replacement = "  \n", x = params$heuristics_details))}
```
...
the sample size in the planned study consists a total of `r params$participants` participants, each contributing `r params$observations` observations. 
